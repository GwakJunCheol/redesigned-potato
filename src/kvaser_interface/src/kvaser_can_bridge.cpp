  ...
  while (true)
  {
    ...
    if (can_reader.isOpen())
    {
      ...
      if (ret == ReturnStatuses::OK)
      {
        ...
        {
          geometry_msgs::WrenchStamped can_pub_msg;

          if (msg.id == 1)
          {
            can_pub_msg.wrench.force.x = 0.01*(256*msg.data[0]+msg.data[1])-300.0;
            can_pub_msg.wrench.force.y = 0.01*(256*msg.data[2]+msg.data[3])-300.0;
            can_pub_msg.wrench.force.z = 0.01*(256*msg.data[4]+msg.data[5])-300.0;
            ...
            if (can_reader.isOpen())
            {
              ...
              if (ret == ReturnStatuses::OK)
              {
                  ...
                  msg.flags.msg_delayed || msg.flags.tx_nack))
                {
                  if (msg.id == 2)
                  {                  
                    can_pub_msg.wrench.torque.x = 0.002*(256*msg.data[0]+msg.data[1])-50.0;
                    can_pub_msg.wrench.torque.y = 0.002*(256*msg.data[2]+msg.data[3])-50.0;
                    can_pub_msg.wrench.torque.z = 0.002*(256*msg.data[4]+msg.data[5])-50.0;
                    can_pub_msg.header.frame_id = "0";
                    can_pub_msg.header.stamp = ros::Time::now();
...
int main(int argc, char** argv)
{
  ...
  can_tx_pub = n.advertise<geometry_msgs::WrenchStamped>("can_tx", 500);
